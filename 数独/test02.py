...
#传参list_frame,是否成功有待考证
        def Onclick(self,list_fram):
            if __name__ == '__main__':
                app = wx.App()  # 初始化
                frame_sle = slect_frame(parent=self, id=-1,data02=list_fram) # 实例MyFrame类，并传递参数
                print(frame_sle.data02)
                # frame_sle.data02=list_fram
                frame_sle.Show()  # 显示窗口
                app.MainLoop()  # 调用主循环方法
        self.bt_easy.Bind(wx.EVT_BUTTON, Onclick(self,list_fram=data_all_op[2]))
...
class slect_frame(wx.Frame):
    ...
    #     if __name__ == '__main__':
    #         app = wx.App()  # 初始化
    #         frame_sle = slect_level_frame(parent=self, id=-1,data01=list_level)  # 实例MyFrame类，并传递参数
    #         frame_sle.Show()  # 显示窗口
    #         app.MainLoop()  # 调用主循环方法
